...
import RiskUnderstandingSkill from './components/RiskUnderstandingSkill';
import QuestionnaireSkill from './components/QuestionnaireSkill';
import ProfileRevealSkill from './components/ProfileRevealSkill';
import {
  DEEP_SPACE_BLUE,
  CORPORATE_NAVY,
  CYBER_TEAL,
...
const ModuleTwo = ({ userData, setUserData, setInvestorProfile, onComplete }) => {
  const [currentSkill, setCurrentSkill] = useState(1);
  const [completedSkills, setCompletedSkills] = useState([]);
  const [riskScore, setRiskScore] = useState(null);

  const skills = [
    { id: 1, name: 'Risk Understanding', component: RiskUnderstandingSkill },
    { id: 2, name: 'Questionnaire', component: QuestionnaireSkill },
    { id: 3, name: 'Profile Reveal', component: ProfileRevealSkill }
  ];
  ...
  const renderCurrentSkill = () => {
    ...
    if (currentSkill === 2) {
      // QuestionnaireSkill needs setRiskScore
      return (
        <SkillComponent
          {...commonProps}
      ...
      // ProfileRevealSkill needs riskScore and setInvestorProfile
      return (
        <SkillComponent
          {...commonProps}
          riskScore={riskScore}
      ...
      // RiskUnderstandingSkill just needs common props
  ...
  return (
    <div>
      ...
      >
        ...
        >
          Who Am I? Discover your personal investor profile through interactive assessment.
        </p>
